...
def register():
	...
	new_user = {
		...
		"settings": {
			...
			"proxy": "",
			"logs": [
				"yellow:yellow:Welcome to Hitter!"
			]
		},
		"role": "stable",
		"invites": {}
...
def purchase():
	if not request.is_json:
		return jsonify({"message": "No JSON received"}), 400
	if request.headers.get("X-Sellpass-Secret") == "BcdsijrKjTQleJJxh72HMGAxUEygIPL=" and request.get_json()["Product"]["UniquePath"] == "HitterV1":
